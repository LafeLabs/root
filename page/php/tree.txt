...
function listfiles($localpath){
    ...
    foreach($files as $filename){
        if($filename != "json" && $filename != "pages" && $filename != "php" && $filename != "html" && $filename != "." && $filename != ".." && is_dir($fullpath."/".$filename)){
            
           $fileandpath = substr($localpath,1)."/".$filename;
           if($fileandpath[0] == "/"){
               $fileandpath = substr($fileandpath,1);
           } 
           echo  "\n<li><a href = \"pageeditor.php?path=".$fileandpath."/\">".$fileandpath."/</a></li>\n";
